...
1. **Content Performance Tracker**: Ingests, cleans, and structures performance metrics.
2. **Virality Prediction Engine**: Random Forest ML model predicting viral potential using only pre-publication features to prevent data leakage.
3. **Audience Sentiment Analyzer**: NLP module utilizing NLTK/TextBlob and spaCy to classify sentiment and detect relatability via linguistic triggers.
4. **A/B Testing Framework**: Statistical validation using Welch's t-test and Mann-Whitney U to identify statistically significant strategic advantages.
5. **Engagement Optimization Recommender**: Data-driven prescriptive analytics generating content strategy recommendations.
6. **Growth Visualization Dashboard**: An interactive Streamlit frontend with Plotly charts.
7. **Trend Forecasting Module**: Time-series exponential smoothing to detect emerging relatable struggles.
